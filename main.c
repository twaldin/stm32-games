...
static void clock_setup(void) { rcc_clock_setup_in_hse_8mhz_out_custom(); }
...
int main(void) {
  ...
  st_draw_string(15, 30, "Press any button to play", ST_COLOR_WHITE,
                 &font_ubuntu_mono_24);

  while (1) {
    if (is_right_pressed() || is_left_pressed() || is_down_pressed() ||
        is_up_pressed()) {
      snake_game();
    }
  }
...
// bool button functions
// Return true if button is pressed
bool is_left_pressed(void) { return gpio_get(GPIOB, GPIO12) == 0; }

bool is_down_pressed(void) { return gpio_get(GPIOB, GPIO13) == 0; }

bool is_right_pressed(void) { return gpio_get(GPIOB, GPIO14) == 0; }

bool is_up_pressed(void) { return gpio_get(GPIOB, GPIO15) == 0; }

void button_setup(void) {
  // Enable clock for the GPIO port where button is connected (B)
  rcc_periph_clock_enable(RCC_GPIOB);

  // initialize buttons (GPIOB 12-15)
  // left
...
// below here is rcc initialization from the driver i used
